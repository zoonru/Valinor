...
final class LexingParserTest extends UnitTestCase
{
    /**
    ...
    public static function parse_valid_types_returns_valid_result_data_provider(): iterable
    {
        ...
        yield 'key-of applied to class array constant' => [
            'raw' => 'key-of<' . ObjectWithConstants::class . '::CONST_WITH_ARRAY_VALUE_A>',
            'transformed' => "'string'|'integer'|'float'",
            'type' => UnionType::class,
        ];
    ...
    public function test_multiple_intersection_types_are_parsed(): void
    {
        ...
        $intersectionType = $this->parse($raw);
    ...
    {
        $type = $this->parse('DateTimeInterface&');
    ...
    {
        $type = $this->parse('array<float, string>');
    ...
    {
        $type = $this->parse('non-empty-array<float, string>');
    ...
    {
        $type = $this->parse('non-empty-array<int string>');
    ...
    {
        $type = $this->parse('non-empty-list<');
    ...
    {
        $type = $this->parse('non-empty-list<string');
    ...
    {
        $type = $this->parse('iterable<int string>');
    ...
    {
        $type = $this->parse('class-string<DateTimeInterface');
    ...
    {
        $type = $this->parse('class-string<stdClass|int>');
    ...
    {
        $type = $this->parse('int&DateTimeInterface');
    ...
    {
        $type = $this->parse('int<string, 1337>');
    ...
    {
        $type = $this->parse(PureEnum::class . '::');
    ...
    {
        $type = $this->parse(PureEnum::class . '::ABC');
    ...
    {
        $type = $this->parse(PureEnum::class . '::F**O');
    ...
    {
        $type = $this->parse(EnumWithNoCase::class);
    ...
    {
        $type = $this->parse(ObjectWithConstants::class . '::');
    ...
    {
        $type = $this->parse(ObjectWithConstants::class . '::ABC');
    ...
    {
        $type = $this->parse(ObjectWithConstants::class . '::F**O');
    ...
    public function test_missing_generic_closing_bracket_throws_exception(): void
    {
        ...
        $type = $this->parse("$genericClassName<string");
    ...
    public function test_missing_comma_in_generics_throws_exception(): void
    {
        ...
        $type = $this->parse("$className<int, string bool>");
    ...
    public function test_superfluous_generic_throws_exception(): void
    {
        ...
        $type = $this->parse("$className<int, string, bool, string>");
    ...
    {
        $type = $this->parse('callable(string, int');
    ...
    private function parse(string $raw): Type
    {
        return $this->getService(TypeParser::class)->parse($raw);
    }
}
